...
{
    public class AiTaskFromChest : AiTaskBase
    {
        bool stop = false;
        Vec3d targetPos;
        ...
        public AiTaskFromChest(EntityAgent entity) : base(entity)
        {
        }
        ...
        public override void LoadConfig(JsonObject taskConfig, JsonObject aiConfig)
        {
            ...
            onlyOnGuard = taskConfig.IsTrue("onlyOnGuard");
        }
        ...
        public override bool ShouldExecute()
        {
            ...
            if (prog.dormant || !entity.LeftHandItemSlot.Empty || prog?.CheckChest() != true || prog.workStack == null || (onlyOnGuard && !prog.CheckArea())) return false;
            Block check = world.BlockAccessor.GetBlock(prog.workChest);
            if (check != null && check.Code.Path.Contains("translocatorchest"))
            {
                if (!prog.TakeFromChest()) failureTime = entity.World.ElapsedMilliseconds + TemporalHackerConfig.Loaded.BotFailureCooldownMs; else prog.ConsumeEnergy(1);
                return false;
            }
