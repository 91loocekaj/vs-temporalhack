...
{
    public class AiTaskGetTool : AiTaskBase
    {
        bool stop = false;
        Vec3d targetPos;
        ...
        float minDist = 1.3f;
        long failureTime;
        EnumTool tool = EnumTool.Knife;

        public AiTaskGetTool(EntityAgent entity) : base(entity)
        {
        }
        ...
        public bool TakeFromChest()
        {
            ...
            if (bc.Inventory != null)
            {
                ...
                {
                    if (slot.Itemstack?.Collectible.Tool != tool) continue;
